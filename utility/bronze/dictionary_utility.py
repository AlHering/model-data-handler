...
from .comparison_utility import *


logger = logging.Logger("[DictionaryUtility]")
...
def check_equality(data: dict, test_data: dict, exceptions: list = [], enable_logging: bool = False) -> bool:
    """
    ...
    :param enable_logging: Output is logged if enable_logging is set to True. Defaults to False.
    ...
    for elem in data:
        if elem in test_data:
            if not any(elem == e[-1] for e in exceptions):
                if isinstance(data[elem], dict):
                    if isinstance(test_data[elem], dict):
                        if not check_equality(data[elem], test_data[elem], [e[1:] for e in exceptions if len(e) > 1 and (e[0] == elem or e[0] == "*")], enable_logging):
                            if enable_logging:
                                logger.info(elem + ": " + str(data[elem]) + " != " + str(test_data[elem]))
                            return False
                    else:
                        if enable_logging:
                            logger.info(elem + ": " + str(data[elem]) + " != " + str(test_data[elem]))
            ...
            if enable_logging:
                logger.info(elem + ": missing " + str(data[elem]))
            return False
    return True
