...
old_ts_format = "%Y_%m_%d-(%H-%M-%S)"
ts_format = "%Y_%b_%d-(%H-%M-%S)"
...
def get_difference(first_timestamp: str, second_timestamp: str, metric: str = "S") -> float:
    ...
    """
    first_timestamp = dt.strptime(first_timestamp, ts_format)
    second_timestamp = dt.strptime(second_timestamp, ts_format)
...
def normalize_timestamp(timestamp: str, default_to_input: bool = False) -> str:
    ...
    """
    for fmt in fmt_profiles:
        if re.fullmatch(fmt_profiles[fmt]["regex"], timestamp):
            return convert_format(fmt_profiles[fmt]["format"], ts_format, timestamp)
    if default_to_input:
        return timestamp
...
def get_past_time(delta_time: datetime.timedelta, fmt: str = ts_format) -> str:
    """
...
def get_future_time(delta_time: datetime.timedelta, fmt: str = ts_format) -> str:
    """
